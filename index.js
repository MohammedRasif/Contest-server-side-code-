...
//const stripe = require('stripe')(process.env.STRIPE_SECRET_KEY);
//const stripe = require('stripe')('sk_test_51PPR4KFtGKLS4NRKJae1Jjk2mu1UW8IKAb3UiNBr4NJnTpOXRNofHIzQDEDlCVb2XR2WQqXoit5CxNLvLfryAT0800rhTGf4jH')
...
app.use(
  cors({
    origin:["http://localhost:5173",
            "https://moviez-bc050.web.app",
            "https://moviez-bc050.firebaseapp.com"
           ],
           credentials:true,
})
);
...
async function run() {
  try {
    ...
    const userCollection = client.db("moviezDb").collection("user");
    const contestCollection = client.db("contestBD").collection("contest")
    const allContestsCollection = client.db("contestBD").collection("allContests")
    const paymentCollection = client.db("contestBD").collection("payment")
    const submitedCollection = client.db("contestBD").collection("submit")
    const winningCollection = client.db("contestBD").collection("winning")
    ...
    // })
      ...
      app.get('/allContests',async(req,res) =>{
        const cursor = allContestsCollection.find();
      ...
      //   const cursor = winningCollection.find();
      //   const result = await cursor.toArray();
      //   res.send(result);
      // })
      ...
      app.get('/winning',async(req,res)=>{
        ...
        const result = await winningCollection.find().toArray();
        console.log(result)
        res.send(result);
      })
